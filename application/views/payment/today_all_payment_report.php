...
<h1>
	<?php echo isset($title) ? $title : 'Today Payement Summary' ; ?></h1>
	...
	<table id="today_transfer" class="table table-bordered table-striped">
			<thead>
				<tr>
					...
					<th width="">Total Received</th>
					<th width="">Total_reciever</th>
					</tr>
			</thead>
			<?php
			if(!$today){
			...
			<tr>
				...
				<td><?php echo  $profile->receive_total?></td>
				<td>

					<a data-toggle="modal" data-target="#view-modal" data-id="<?php echo $profile->receiving_branch?>" id="getUser" class="btn btn-sm btn-view"><?php echo  $profile->Total_reciever?></a>
				</td>
			</tr>

			<?php
			 $count++;
			$total=$total+$profile->receive_total;
		}
			?>
   ...
   <div class="modal-dialog modal-lg">
      <div class="modal-content">
         ...
         <div class="modal-header">
             ...
             <i class="glyphicon glyphicon-user"></i>Todays Message In
             </h4>
         </div>
...
$(document).ready(function(){
  ...
  //  $('#todays-transfer tr').remove();
     ...
     $.ajax({
		    	url:'<?php  echo base_url()?>payment/getTodayBranch',
      	 type: 'POST',
         data: {'uid':uid},
         dataType: 'json'
     })
     .done(function(data){
            ...
            var row = $('<tr><td>'+data[i].secuirity_code+'</td><td>'+data[i].first_name+data[i].first_name+data[i].first_name+'</td><td>'+data[i].rcvr_fname+" "+data[i].rcvr_mname+" "+data[i].rcvr_lname+'</td><td>'+data[i].amount+'</td><td>'+data[i].receiving_branch+'</td><td>'+data[i].sending_branch+'</td></tr>');
